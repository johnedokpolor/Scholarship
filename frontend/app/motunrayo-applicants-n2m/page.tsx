...
const Page = () => {
  ...
  const [motunrayo, setMotunrayo] = useState([]);
  const [paid, setPaid] = useState(false);
  const [loading, setLoading] = useState(true);

  const paidApplicants = motunrayo.filter((applicant: any) => applicant.paid);
    ...
    const fetchApplicants = async () => {
      ...
      setMotunrayo(
        response.data.applicants.filter(
          (applicant: any) => applicant.referredBy === "MOTUNRAYO"
        )
      );
    };
    setLoading(false);
      ...
      <div className="bg-white text-center flex gap-3 items-center justify-center p-3 rounded-lg">
        <div>
          <span className="font-bold">
            No Of Motunrayo&apos;s {paid ? "Paid" : ""} Applicants:{" "}
          </span>
          {motunrayo.length} applicants
        </div>
        <button
          onClick={togglePaid}
