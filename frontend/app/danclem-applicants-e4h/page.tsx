...
const Page = () => {
  ...
  const [danclem, setDanclem] = useState([]);
  const [paid, setPaid] = useState(false);
  const [loading, setLoading] = useState(true);

  const paidApplicants = danclem.filter((applicant: any) => applicant.paid);
    ...
    const fetchApplicants = async () => {
      ...
      setDanclem(
        response.data.applicants.filter(
          (applicant: any) => applicant.referredBy === "DANCLEM"
        )
      );
    };
    setLoading(false);
      ...
      <div className="bg-white text-center flex gap-3 items-center justify-center p-3 rounded-lg">
        <div>
          <span className="font-bold">
            No Of Danclem&apos;s {paid ? "Paid" : ""} Applicants:{" "}
          </span>
          {danclem.length} applicants
        </div>
        <button
          onClick={togglePaid}
